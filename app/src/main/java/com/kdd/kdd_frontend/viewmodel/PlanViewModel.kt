...
class PlanViewModel : ViewModel() {
    ...
    fun cargarHistorial() {
        viewModelScope.launch {
            ...
            try {
                val response = ApiClient.api.getHistorialPlanes()
                if (response.isSuccessful) {
                    val planes = response.body()?.map { it.toPlanCardData() } ?: emptyList()
        ...
        viewModelScope.launch {
            try {
                ...
                if (response.isSuccessful) {
                    ...
                    onError("No se pudo borrar el plan (${response.code()})")
                }
            } catch (e: Exception) {
        ...
        viewModelScope.launch {
            try {
                ...
                if (response.isSuccessful) {
                    ...
                    onError("No se pudo editar el plan (${response.code()})")
                }
            } catch (e: Exception) {
                onError("Error de conexión")
            }
        ...
        viewModelScope.launch {
            try {
                val ref = FirebaseStorage.getInstance().reference
                    .child("planes/$planId/portada.jpg")
                ref.putFile(uri).await()
                val url = ref.downloadUrl.await().toString()
                val response = ApiClient.api.actualizarFotoPlan(planId, mapOf("imagenUrl" to url))
                if (response.isSuccessful) {
                    cargarDetalle(planId)
                    onSuccess()
                } else {
                    onError("Error al guardar la foto (${response.code()})")
                }
            } catch (e: Exception) {
                onError("Error al subir la foto")
            }
        }
    }
}
